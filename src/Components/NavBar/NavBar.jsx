        ...
        <>
            <nav className='navContainer lg-row d-flex gx-0'>
                ...
                <div className='navlinks  col-lg-6 d-lg-flex  justify-content-lg-end justify-content-center align-items-center'>
                    ...
                    <NavLink>Specification</NavLink>
                    <NavLink >FAQ`s</NavLink>
                </div>
                <div className='callus col-lg-3 p-0 justify-content-center align-items-center bg-white'>Call us: <span>084 234 56 789</span> </div>
                <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.99 }} transition={{ type: "spring", stiffness: 400, damping: 7, duration: 0.2 }} className="menu navbar-toggler" onClick={isOpen}>
            ...
            <AnimatePresence className={`mobile`} >
                {
                    open && (
                        <motion.div className="dropdown fs-18" variants={item} initial={{ height: 0, opacity: 0 }} animate={{ height: "500px", opacity: 1 }} transition={{ duration: .5 }} exit={{ height: 0, opacity: 0, delay: 0 }}>
                            ...
                            <motion.div className='m-2 curs' onClick={() => navigation('/about')} initial={{ y: 80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: .7 }}
                            ...
                            <motion.div className='m-2 curs' onClick={() => navigation('/projects')} initial={{ y: 80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: .6 }}
                                exit={{ opacity: 0, y: 90, transition: { ease: "easeInOut", delay: .6 } }}
                            >
                                <motion.div className='m-2 curs' whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.9 }} transition={{ type: "spring", stiffness: 400, damping: 5 }}>{t("our-projects")}</motion.div>

                            </motion.div>
                            <motion.div className='m-2 curs' onClick={() => navigation('/certificates')} initial={{ y: 80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: .5 }}
                                exit={{ opacity: 0, y: 90, transition: { ease: "easeInOut", delay: .4 } }}>
                                <motion.div className='m-2 curs' whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.9 }} transition={{ type: "spring", stiffness: 400, damping: 5 }}>{t('certificates')}</motion.div>
                            ...
                            >
                                <button className="btn  dropdown-toggle" type="button" data-bs-toggle="dropdown" disabled aria-expanded="false">
                                    {lang === 'en'
                                        ? <> <img style={{ width: "25px", marginRight: "7px", borderRadius: "2px" }} src={usf} alt="sa" />En </>
                                        : <> <img style={{ width: "25px", marginRight: "7px", borderRadius: "2px" }} src={uae} alt="sa" /> Ar </>
                                    }
                                </button>
